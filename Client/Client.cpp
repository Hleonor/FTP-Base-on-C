...
void get_file(SOCKET sockfd)
{
    ...
    fd = fopen(filePath, "wb"); //使用二进制方式写入
    if (fd == NULL)
    {
        printf("无法打开文件%s，写入失败！\n", cmd_packet.param);
        return;
    }
    ...
    if (rspns_packet.rspnsid == ERR)
    {
        ...
        fclose(fd);
        // 删除文件: 在下载文件过程中，如果出现错误导致下载中断
        // 这时已经成功创建的本地文件需要被删除以避免文件数据残留，从而保证文件完整性。
        // 因此，如果下载过程中出现错误，代码中使用了Windows API的DeleteFile函数来删除已经创建的本地文件。
